...
class FullNetwork(nn.Module):
    """
    Module implementing the same baseline CNN architecture as the
    Defensive Distillation paper (https://arxiv.org/pdf/1511.04508.pdf).
    """
...
class TeacherModel(L.LightningModule):
    ...
    def configure_optimizers(self):
        # slightly modified training hyperparams from DD paper to better suit pytorch (same results)
...
def get_CIFAR10_loaders(batch_size):
    """
    Read the CIFAR10 dataset as pytorch DataLoaders.
    Params:
        batch_size: int, batch size to use for the DataLoaders.
    Returns:
        Pytorch DataLoaders for the training set (50000) and test set (10000).
    """
    # define simple transform to use for CIFAR10
